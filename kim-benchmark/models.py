...
class MultiConv(nn.Module):
    ...
    def __init__(self, W, min_filter, max_filter, num_filter_maps, s):
        ...
        self.embed.weight.data = W
        self.embed_drop = nn.Dropout(p=DROPOUT_EMBED)

        self.convs = []
        for idx,filter_size in enumerate(range(min_filter, max_filter+1)):
            conv = nn.Conv1d(W.size()[1], num_filter_maps, kernel_size=filter_size)
            conv.weight.data.normal_(-0.01, 0.01)
    ...
    def forward(self, x):
        ...
        for i in range(len(self.convs)):
            conv = self.convs[i]
            conv_output = self.conv_drop(conv(x))
            pool_output = F.relu(F.max_pool1d(conv_output, kernel_size=conv_output.size()[2]))
    ...
    def enforce_norm_constraint(self):
        self.fc.weight.data.renorm(p=2, dim=0, maxnorm=self.s**2)

class VanillaConv(nn.Module):

    def __init__(self, W, kernel_size, num_filter_maps):
        ...
        self.conv.weight.data.normal_(-0.01, 0.01)
...
#        x = self.conv_drop(F.max_pool1d(x, kernel_size=x.size()[2]))
