...
def main(Y, vocab_min, model_path, dataset):
	...
	#cnn = build_model(Y, old_version=old_version)
	cnn = build_model_multiwindow(Y, 3, 5, 1, old_version=old_version)
	#cnn = build_model_multiconv(Y, old_version)
...
def build_model(Y, old_version=False):
	...
	cnn.add(Embedding(VOCAB_SIZE, EMBEDDING_SIZE, dropout=DROPOUT_EMBED, input_length=MAX_LENGTH))
	...
	if old_version:
		...
		cnn.add(MaxPooling1D(pool_length=MAX_LENGTH-FILTER_SIZE+1))
		cnn.add(Reshape((Y*CONV_DIM_FACTOR,)))
	else:
...
def build_model_multiwindow(Y, s, l, step, old_version=False):
	...
	for i,sz in enumerate(range(s, l+1, step)):
		...
		if old_version:
			#http://stats.stackexchange.com/questions/257321/what-is-global-max-pooling-layer-and-what-is-its-advantage-over-maxpooling-layer
			from keras.layers import MaxPooling1D, Reshape
			pools.append(MaxPooling1D(pool_length=MAX_LENGTH-FILTER_SIZE+1)(convs[i]))
			reshapes.append(Reshape((Y*CONV_DIM_FACTOR,))(pools[i]))
		else:
			from keras.layers.pooling import GlobalMaxPooling1D
...
def build_model_multiconv(Y, old_version=False):
	"""
		Builds the single window CNN model with two sequential conv-pool layers
		params:
			Y: size of the label space
		returns:
			cnn: the CNN model
	...
	from keras.layers.pooling import GlobalMaxPooling1D
	cnn.add(MaxPooling1D(pool_length=POOL_LENGTH))
	cnn.add(Convolution1D(Y*CONV_DIM_FACTOR, FILTER_SIZE, activation='tanh'))
	cnn.add(GlobalMaxPooling1D())
